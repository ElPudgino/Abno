...
import pzi.abno.entity.EntityAbno;
import pzi.abno.entity.ModelEntityAbno;
...
public class RenderEntityAbno extends GeoEntityRenderer<EntityAbno> {
    public RenderEntityAbno(RenderManager renderManager, ModelEntityAbno model) {
        super(renderManager, model);
    }

    public float HeadPitchModify(EntityAbno entity, float headpitch)
    {
        if (entity.looking)
        {
            double d0 = entity.posX - entity.lookPosX;
            double d1 = entity.posY + entity.getEyeHeight() - entity.lookPosY;
            double d2 = entity.posZ - entity.lookPosZ;
    ...
    public float HeadYawModify(EntityAbno entity, float headyaw)
    {
        if (entity.looking)
        {
            double d0 = entity.posX - entity.lookPosX;
    ...
    @Override
    public void doRender(EntityAbno entity, double x, double y, double z, float entityYaw, float partialTicks)
    {
        ...
        netHeadYaw = HeadYawModify(entity, netHeadYaw);
        ...
        headPitch = HeadPitchModify(entity, headPitch);
		...
		if (modelProvider instanceof IAnimatableModel) {
			((IAnimatableModel<EntityAbno>) modelProvider).setLivingAnimations(entity, this.getUniqueID(entity), predicate);
		}
		...
		for (GeoLayerRenderer<EntityAbno> layerRenderer : this.layerRenderers) {
        ...
        PostRender(entity, x, y, z, entityYaw, partialTicks);
